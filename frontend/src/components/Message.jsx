
import React from 'react';

const Message = ({ variant, children, title, dismissible, onDismiss, className = '' }) => {
  const getVariantClasses = () => {
    switch (variant) {
      case 'success':
        return {
          container: 'bg-green-50 border-green-200 text-green-800',
          icon: 'text-green-400',
          button: 'text-green-400 hover:text-green-600 hover:bg-green-100',
          title: 'text-green-800'
        };
      case 'danger':
        return {
          container: 'bg-red-50 border-red-200 text-red-800',
          icon: 'text-red-400',
          button: 'text-red-400 hover:text-red-600 hover:bg-red-100',
          title: 'text-red-800'
        };
      case 'warning':
        return {
          container: 'bg-yellow-50 border-yellow-200 text-yellow-800',
          icon: 'text-yellow-400',
          button: 'text-yellow-400 hover:text-yellow-600 hover:bg-yellow-100',
          title: 'text-yellow-800'
        };
      case 'info':
      default:
        return {
          container: 'bg-blue-50 border-blue-200 text-blue-800',
          icon: 'text-blue-400',
          button: 'text-blue-400 hover:text-blue-600 hover:bg-blue-100',
          title: 'text-blue-800'
        };
    }
  };

  const getIcon = () => {
    switch (variant) {
      case 'success':
        return (
          <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
          </svg>
        );
      case 'danger':
        return (
          <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
          </svg>
        );
      case 'warning':
        return (
          <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
          </svg>
        );
      case 'info':
      default:
        return (
          <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
          </svg>
        );
    }
  };

  const variantClasses = getVariantClasses();

  return (
    <div className={`border rounded-lg p-4 ${variantClasses.container} ${className}`} role="alert">
      <div className="flex items-start">
        {/* Icon */}
        <div className={`flex-shrink-0 mr-3 ${variantClasses.icon}`}>
          {getIcon()}
        </div>
        
        {/* Content */}
        <div className="flex-1">
          {title && (
            <h3 className={`text-sm font-medium mb-1 ${variantClasses.title}`}>
              {title}
            </h3>
          )}
          <div className="text-sm">
            {children}
          </div>
        </div>
        
        {/* Dismiss Button */}
        {dismissible && (
          <button
            onClick={onDismiss}
            className={`ml-3 flex-shrink-0 rounded-md p-1.5 transition-colors duration-200 ${variantClasses.button}`}
            aria-label="Dismiss"
          >
            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
              <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
            </svg>
          </button>
        )}
      </div>
    </div>
  );
};

// Specialized message components for common use cases
export const SuccessMessage = ({ children, ...props }) => (
  <Message variant="success" {...props}>
    {children}
  </Message>
);

export const ErrorMessage = ({ children, ...props }) => (
  <Message variant="danger" {...props}>
    {children}
  </Message>
);

export const WarningMessage = ({ children, ...props }) => (
  <Message variant="warning" {...props}>
    {children}
  </Message>
);

export const InfoMessage = ({ children, ...props }) => (
  <Message variant="info" {...props}>
    {children}
  </Message>
);

Message.defaultProps = {
  variant: 'info',
  dismissible: false,
  className: ''
};

export default Message;
